...
let playing = false, score = 0, seconds = 30, playerX = 0.5, keys = {}, stars = [], spawnTimer, ticker, animation, muted = false, audioCtx, frozenUntil = 0, freezeTimer, damageTimer, fireworksShown = false;
...
function cropAccessory(source, x, y, width, height, outline, target) {
  ...
  const isTargetColor = (red, green, blue) => {
    ...
    if (target === 'tree') return green > red * 1.12 && green > blue * 1.08;
    return red > green * 1.18 && red > blue * 1.12;
  };
  ...
  for (let index = 0; index < data.length; index += 4) {
    ...
    if (isHatYellow || !insideOutline(px, py) || (!primary[pixel] && !(isDarkLine && touchesPrimary(px, py)))) data[index + 3] = 0;
  }
  context.putImageData(pixels, 0, 0);
...
const makeCharacterCutout = () => {
  if (!characterImage.dataset.cutout && characterImage.naturalWidth) {
    ...
    collectibleSprites.fox = cropAccessory(cutout, 110, 20, 150, 150, [[61,27],[77,20],[89,35],[94,57],[116,55],[124,70],[111,81],[127,91],[115,104],[104,112],[95,124],[73,139],[48,146],[25,137],[14,120],[14,99],[28,87],[43,79],[47,59],[37,44]], 'fox');
    collectibleSprites.tree = cropAccessory(cutout, 290, 134, 120, 118, [[15,8],[37,1],[57,12],[67,27],[91,28],[111,46],[119,65],[103,75],[98,97],[79,115],[59,104],[46,87],[27,80],[9,62],[1,37]], 'tree');
...
function spawn() {
  ...
  const item = { el: star, x: .06 + Math.random() * .84, y: -60, speed: 2.1 + Math.random() * 1.8 + (30 - seconds) * .045, points, hazard, kind };
  star.style.left = `calc(${item.x * 100}% - 25px)`; game.append(star); stars.push(item);
}
function isFrozen() { return Date.now() < frozenUntil; }
function takeDamage() { frozenUntil = Date.now() + 2000; score = Math.max(0, score - 3); scoreEl.textContent = score; character.classList.add('frozen'); damageEl.classList.add('show'); clearTimeout(freezeTimer); clearTimeout(damageTimer); freezeTimer = setTimeout(() => character.classList.remove('frozen'), 2000); damageTimer = setTimeout(() => damageEl.classList.remove('show'), 2000); foxSound(); }
function frame() { if (!playing) return; const g = bounds(), p = character.getBoundingClientRect(); if (!isFrozen() && (keys.ArrowLeft || keys.a)) setPlayer(playerX - .012); if (!isFrozen() && (keys.ArrowRight || keys.d)) setPlayer(playerX + .012); for (const item of [...stars]) { item.y += item.speed; item.el.style.transform = `translateY(${item.y}px) rotate(${item.y / 7}deg)`; const s = item.el.getBoundingClientRect(); const hit = s.bottom > p.top + 17 && s.top < p.bottom - 25 && s.right > p.left + 20 && s.left < p.right - 20; if (hit) { if (item.hazard) takeDamage(); else { score += item.points; scoreEl.textContent = score; if (item.kind === 'tree') rustleSound(); else if (item.kind === 'petal') shamisenSound(); else if (item.kind === 'red-star') redStarSound(); else yellowStarSound(); } item.el.animate([{transform: item.el.style.transform, opacity: 1}, {transform: `${item.el.style.transform} scale(1.8)`, opacity: 0}], {duration: 180}); removeStar(item); } else if (item.y > g.height + 50) removeStar(item); } animation = requestAnimationFrame(frame); }
function removeStar(item) { item.el.remove(); stars = stars.filter(s => s !== item); }
function start() { stars.forEach(s => s.el.remove()); stars = []; score = 0; seconds = 30; playerX = .5; fireworksShown = false; fireworks.replaceChildren(); frozenUntil = 0; clearTimeout(freezeTimer); clearTimeout(damageTimer); character.classList.remove('frozen', 'facing-left'); damageEl.classList.remove('show'); scoreEl.textContent = '0'; timeEl.textContent = seconds; setPlayer(playerX); playing = true; message.classList.add('hidden'); startButton.textContent = 'プレイ中！'; startButton.classList.add('playing'); game.focus(); spawn(); spawnTimer = setInterval(spawn, 700); ticker = setInterval(() => { seconds = Math.max(0, seconds - 1); timeEl.textContent = seconds; if (seconds === 0) end(); }, 1000); animation = requestAnimationFrame(frame); }
function end() { playing = false; seconds = 0; timeEl.textContent = seconds; frozenUntil = 0; clearTimeout(freezeTimer); clearTimeout(damageTimer); character.classList.remove('frozen'); damageEl.classList.remove('show'); clearInterval(spawnTimer); clearInterval(ticker); spawnTimer = ticker = null; cancelAnimationFrame(animation); stars.forEach(s => s.el.remove()); stars = []; const greatScore = score >= 50; if (greatScore) launchFireworks(true); startButton.textContent = 'もういちど！'; startButton.classList.remove('playing'); message.innerHTML = `${greatScore ? 'がんばったね！' : 'おしまい！'}<br><strong>${score} 点</strong><small>タップして もういちど あそぼう！</small>`; message.classList.remove('hidden'); beep(350, .25); }
function moveByButton(direction) { if (!playing || isFrozen()) return; setPlayer(playerX + direction * .09); }
startButton.addEventListener('click', start); message.addEventListener('click', start); game.addEventListener('pointermove', e => { if (playing && !isFrozen() && e.buttons) setPlayer((e.clientX - bounds().left) / bounds().width); }); game.addEventListener('pointerdown', e => { if (playing && !isFrozen()) setPlayer((e.clientX - bounds().left) / bounds().width); });
...
leftButton.addEventListener('click', () => moveByButton(-1)); rightButton.addEventListener('click', () => moveByButton(1)); soundButton.addEventListener('click', () => { muted = !muted; soundButton.textContent = muted ? '🔇' : '♪'; soundButton.setAttribute('aria-label', muted ? '効果音をオンにする' : '効果音をオフにする'); }); window.addEventListener('resize', () => setPlayer(playerX)); setPlayer(.5);
