...
let playing = false, score = 0, seconds = 30, playerX = 0.5, keys = {}, stars = [], spawnTimer, ticker, animation, muted = false, audioCtx, frozenUntil = 0, freezeTimer, damageTimer, surpriseTimer, fireworksShown = false;
...
const makeCharacterCutout = () => {
  if (!characterImage.dataset.cutout && characterImage.naturalWidth) {
    ...
    collectibleSprites.tree = cropAccessory(cutout, 280, 125, 140, 140, [[0,0],[140,0],[140,140],[0,140]], 'tree');
...
bindMoveButton(leftButton, -1); bindMoveButton(rightButton, 1); soundButton.addEventListener('click', () => { muted = !muted; soundButton.textContent = muted ? '🔇' : '♪'; soundButton.setAttribute('aria-label', muted ? '効果音をオンにする' : '効果音をオフにする'); }); window.addEventListener('resize', () => setPlayer(playerX)); setPlayer(.5);
