...
let playing = false, score = 0, seconds = 30, playerX = 0.5, keys = {}, stars = [], spawnTimer, ticker, animation, muted = false, audioCtx, frozenUntil = 0, freezeTimer, damageTimer, surpriseTimer, bashfulTimer, fireworksShown = false;
...
function takeDamage() { frozenUntil = Date.now() + 2000; score = Math.max(0, score - 3); scoreEl.textContent = score; character.classList.add('frozen'); damageEl.classList.add('show'); clearTimeout(freezeTimer); clearTimeout(damageTimer); freezeTimer = setTimeout(() => { if (!isFrozen()) unfreeze(); }, 2050); damageTimer = setTimeout(() => damageEl.classList.remove('show'), 2050); foxSound(); }
