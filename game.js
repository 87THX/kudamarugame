...
function takeDamage() { frozenUntil = Date.now() + 2000; score = Math.max(0, score - 3); scoreEl.textContent = score; character.classList.add('frozen'); damageEl.classList.add('show'); clearTimeout(freezeTimer); clearTimeout(damageTimer); freezeTimer = setTimeout(unfreeze, 2000); damageTimer = setTimeout(() => damageEl.classList.remove('show'), 2000); foxSound(); }
