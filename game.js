...
function takeDamage() { frozenUntil = Date.now() + 2000; score = Math.max(0, score - 3); scoreEl.textContent = score; character.style.removeProperty('filter'); character.classList.remove('star-glow', 'frozen'); void character.offsetWidth; character.classList.add('frozen'); stoneAnimation?.cancel(); stoneAnimation = null; damageEl.classList.add('show'); clearTimeout(freezeTimer); clearTimeout(damageTimer); freezeTimer = window.setTimeout(unfreeze, 2000); damageTimer = window.setTimeout(() => damageEl.classList.remove('show'), 2000); foxSound(); }
